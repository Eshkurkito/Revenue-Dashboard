...
def render_resumen_comparativo(raw: pd.DataFrame | None = None):
    ...
    # --- Sidebar: renderizar siempre pero usando keys determinísticas (evita duplicados) ---
    module_key = MODULE_KEY
    cutoff_key = f"{MODULE_KEY}_cutoff"
    period_prefix = f"{MODULE_KEY}_period"
    props_prefix = f"{MODULE_KEY}_props"

    # Renderizar el sidebar en cada run pero con keys fijas: evita widgets duplicados y evita sidebar vacío
    with st.sidebar:
        st.markdown("### Parámetros – Resumen comparativo")
        cutoff_rc = st.date_input("Fecha de corte", value=today, key=cutoff_key)

        # period inputs (intentar usar helper, si falla caer a inputs manuales)
        try:
